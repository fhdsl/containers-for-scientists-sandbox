...
```
## Activity 0: Set Up Podman and workshop files

<input type="checkbox"> Download the files for this activity by going here: https://github.com/fhdsl/containers-for-scientists-sandbox  
<input type="checkbox"> Click the green button that says "Code" and choose zip file.  
<input type="checkbox"> Download the files for this activity by going here: https://github.com/fhdsl/containers-for-scientists-sandbox  
...
<input type="checkbox"> To run a script using the podman container we could just add reference to a script at the end:  
```
...
<input type="checkbox"> Now we can run the folowing command but we will have to run `podman ps` and get the container ID we need to put here.   
